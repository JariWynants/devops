...
add_image(){
	...
	if [[ -z $RESERVED_IP_ADDRESS ]]; then
		if [[ -z `gcloud compute address list --filter "coi-address"` ]]; then
	...
	gcloud compute instances create $SERVERNAME --machine-type=$SERVERTYPE --image-project=ubuntu-os-cloud --image-family=ubuntu-1804-lts --zone=$ZONE --address=$RESERVED_IP_ADDRESS --metadata-from-file=startup-script=/home/jari/coi-git/startup.sh &> $HOME/coi-git/deployip.log
	echo ""
